...
@IBDesignable class CounterView: UIView
{
    ...
    override func draw(_ rect: CGRect)
    {
        ...
        if counter > 0 {
            ...
            let outlinePath = UIBezierPath(arcCenter: center, radius: bounds.width/2 - 2.5,
                                           startAngle: startAngle, endAngle: outlineEndAngle, clockwise: true); // draw the outer arc
            outlinePath.addArc(withCenter: center, radius: bounds.width/2 - arcWidth + 2.5,
                                         startAngle: outlineEndAngle, endAngle: startAngle, clockwise: false); // draw the inner arc
            outlinePath.close(); // close the outline path
    ...
    */
    func drawMarker(_ context: CGContext, rect: CGRect)
    {
        ...
        {
            context.saveGState()    // 4. save the centred context
