...
class MedalView: UIImageView
{
	...
	*/
    // create an off-screen image. Means if you start drawing outside of draw(_:) or view's gc, you'll have to create your own gc.
    fileprivate func createMedalImage() -> UIImage
    {
        ...
        UIGraphicsBeginImageContextWithOptions(size, false, 0.0) // creates a new gc, true is opaque (faster), false is transparency
        let context = UIGraphicsGetCurrentContext() // get the image's context
		...
		// When you draw an object into the context, this code creates a shadow for each object. Your medal comprises five objects. No wonder it looks a little fuzzy. 
		// Simply group drawing objects with a transparency layer, and you’ll only draw one shadow for the whole group.
        context?.beginTransparencyLayer(auxiliaryInfo: nil)   /// start a transparency layer for group drawing objects
        
        // Object 1. The Lower/back ribbon/polygon (red)
        ...
        context?.drawLinearGradient(gradient!, start: CGPoint(x: 40, y: 40), end: CGPoint(x: 100, y: 160),
        ...
        var transform = CGAffineTransform(scaleX: 0.8, y: 0.8)        // scale transform, scales down 80% of its original size
        ...
        // Object 4. The Upper/front ribbon (blue)
        ...
        // Object 5. The number 1 (dark gold)
        ...
        UIGraphicsEndImageContext()		// close the image's gc
        return image!
    }
