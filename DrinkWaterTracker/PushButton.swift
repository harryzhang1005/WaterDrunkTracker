...
{
    @IBInspectable var fillColor: UIColor = UIColor.greenColor()    // @IBInspectable property will be added to storybord
    ...
    Note that a context has a single path in use at any time: a path is not part of the graphics state.
    ...
    */
    override func drawRect(rect: CGRect)
    {
        ...
        var plusOrMinusPath = UIBezierPath();
        ...
        plusOrMinusPath.moveToPoint(CGPoint(x: self.bounds.width/2 - plusOrMinusLength/2, y: self.bounds.height/2))
        plusOrMinusPath.addLineToPoint(CGPoint(x: self.bounds.width/2 + plusOrMinusLength/2, y: self.bounds.height/2))
        
        if isAddButton      // vertical stroke
        {
            plusOrMinusPath.moveToPoint(CGPoint(x: bounds.width/2, y: bounds.height/2 - plusOrMinusLength/2))
            plusOrMinusPath.addLineToPoint(CGPoint(x: bounds.width/2, y: bounds.height/2 + plusOrMinusLength/2))
        }
